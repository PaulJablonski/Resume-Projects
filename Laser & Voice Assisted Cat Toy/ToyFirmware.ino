...
void loop() {
  switch (curState) {
    ...
    case STATE_TURNACCEL_MOVE: {
      ...
      // Random turning
      if (accelRate > 0 && accelRate < 1500) {
        digitalWrite(DIR_M1, HIGH);
        digitalWrite(DIR_M2, LOW);

        // Varying LED blink rates
        if (millis() % accelRate > floor(accelRate / 2)) {
          if (digitalRead(STEP_M1) == HIGH) {
            digitalWrite(STEP_M1, LOW);
            digitalWrite(STEP_M2, LOW);
            digitalWrite(LED, LOW);
      ...
      } else if (accelRate > 1500 && accelRate < 3000) {
        digitalWrite(DIR_M1, LOW);
        digitalWrite(DIR_M2, HIGH);
